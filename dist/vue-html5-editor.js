...
(function (global, factory) {
  ...
  var dashboard$3 = {
    ...
    methods: {
      ...
      process: function process() {
        ...
        if (config.compress) {
          ...
          lrz_all_bundle(file, config.compress).then(function (rst) {
            if (config.upload) {
              component.uploadToServer(rst.file, rst.base64);
            } else {
      ...
      uploadToServer: function uploadToServer(file, base64) {
        var this$1 = this;
        var config = this.$options.module.config;
        if(config.upload.useOss){
          config.upload.ossUpload(this$1, Command, config, base64, file)
          return;
        }
